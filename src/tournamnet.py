...
class Fight:
    ...
    def get_players(heroes: list[BaseHero]) -> list[BaseHero] | None:
        if len(heroes) <= 1:
            return print(
                "Нет участников для проведения турнира. Прорекламируйте ваш турнир:)"
            )
    ...
    @staticmethod
    def duel(
        player1: Mage | Archer | Warrior, player2: Mage | Archer | Warrior
    ) -> Mage | Archer | Warrior:
    ...
    def print_hp(player1, player2):
        print(
            Fore.GREEN
            + f"Здоровье игрока {player1.name} {player1.nickname}: {player1.hp}"
        )
        print(
            Fore.GREEN
            + f"Здоровье игрока {player2.name} {player2.nickname}: {player2.hp}"
        )

    @staticmethod
    def winner(player):
        print(
            f"Победитель турнира: {player.name} {player.nickname} {player.hp}"
        )
